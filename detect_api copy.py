...
def video_feed(sock):
    ...
    """  if not request.files.getlist('myfile'):
        ...
        opt.save_txt = None
        opt.source = url  """
    opt.source, opt.save_txt = update_options(sock.receive())
    # sock.send(json.dumps([{"name": "Random Int 1", "number": 4944}, {"name": "Random Int 2", "number": 1096}, {"name": "Random Int 3", "number": 2210}]))
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        if save_txt:
            ...
            # dict_result["results"]=result
            dict_result['img'] =base64.b64encode(im0).decode('utf-8')
            dict_result["info"]=f"{s}{'' if len(det) else '(no detections), '}{dt[1].dt * 1E3:.1f}ms"

            # sock.send(json.dumps([{"name": "Random Int 1", "number": seen}, {"name": "Random Int 2", "number": seen}, {"name": "Random Int 3", "number": 2210}]))
